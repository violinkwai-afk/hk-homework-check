...
**Batch 2, built 2026-09-25, from a real P5 1st-term exam.** Downloaded and read directly (not via sub-agent this time, specifically so exact wording is verbatim and safe to build regexes against): p1-p6.com "二零二五至二零二六年度上學期 五年級 數學科 考試". This one exam confirmed 4 of the survey's ~35 findings with real evidence, all 4 now built:

| Type | Tier | Status | Example (real, verbatim from source) | Notes |
|---|---|---|---|---|
| Large-magnitude Chinese numeral -> Arabic numeral (萬/億) | A | **built** (`verifyChineseLargeNumeralToArabic` + `parseChineseLargeNumber`) | "以阿拉伯數字寫出「五億零八百萬零二十」。" (=508000020) | Q8. Separate parser from `parseChineseNumberWord` (kept at its existing 0-99 scope for its other callers) -- this one is section-based (splits on 億/萬, each section parsed via 千/百/十 place words) |
| Repeated-digit place-value difference | A | **built** (`verifyRepeatedDigitPlaceValueDifference`) | "在71460864這個數中，兩個「6」的數值相差多少？" (=59940) | Q9. Only fires when the digit appears exactly twice in the printed number |
| Substitute-and-evaluate (single-letter variable) | A | **built** (`verifySubstituteAndEvaluate`) | "如果T=8，那麼10+T-6的值是___" (=12); "如果F=4，那麼3F÷2的值是___" (=6, implicit multiplication "3F") | Algebra section Q5/Q6 |
| Fraction/mixed-number ascending sort | A, structured-input | **built** (`verifySortFractionsAscending`) | "把以下各分數由小至大排列出來。37/5，7又7/9，7又2/3" | Q10. Same "candidate values as structured input, not raw OCR text extraction" design as `verifySelectTwoNumbersSumTarget` -- the printed candidates are stacked visual fractions in the source image |

Also confirmed (not newly built, already covered): odd/even MC reasoning about a fraction product range (Q16/17), "write an algebraic expression from a word description" (algebra Q1-4, still flagged parsing-hard, not attempted), "which is NOT an algebraic expression" MC and "which expression differs in value from 4n" MC (Q7/8, still blocked on the general MC-answer-representation integration decision noted at the top of this verifier section, same as every other MC-shaped finding in this file) -- none of these built this pass, logged for the next batch.
