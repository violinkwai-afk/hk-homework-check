...
- 🟨 **第5項：加Google Vision數題號嘅機制，捕捉跌漏內容（2026-09-25更新，完整定案版本）——已寫code，淨係做緊log，未變真正用戶見到嘅提示。**
...
- 🟨 **第10項：GLM-4.6V（Z.ai）已經做咗一次真實3張相測試（Qwen baseline vs GLM-4.6V並排），結果未確定。** 客觀觀察：GLM喺3張入面2張讀到嘅題數明顯少過Qwen（6 vs 10、1 vs 6），但未逐句核實內容，唔知係漏讀定係佢分題方式唔同，要用返完整rigor流程（人手逐句核對）先有結論。
- 🔲 **第11項：搵到多3個新candidate，全部未測試。** GLM-5.3-Flash（Z.ai，同GLM-4.6V同公司，更平）、GPT-6 Luna（OpenAI穩陣大廠、快平tier）、Ling-3.0-Flash-VL（InclusionAI，公司冇咁出名但夠平，明文vision-language）。三個都冇試過，等緊你揀要試邊個/幾多個。
