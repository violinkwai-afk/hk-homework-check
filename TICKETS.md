...
- 🟡 **第21項（2026-09-26真實發現，等緊用戶確認）：Qwen嘅call冇排除Alibaba做provider，小朋友功課相有機會經阿里雲伺服器處理。** 查真實OpenRouter資料，`qwen/qwen3-vl-235b-a22b-instruct`背後有5間provider可以揀：DeepInfra、Venice、Parasail、**Alibaba**、Novita——而家code冇set `provider:{ignore:["Alibaba"]}`，同DeepSeek嗰個call已經有嘅設定唔一致（DeepSeek嗰個排除Alibaba嘅原因係content審查誤判，唔係私隱，但客觀效果一樣）。用戶關注返內地私隱問題，已經問緊用戶要唔要加返呢個排除設定。細改動，等緊確認就可以即刻做。
