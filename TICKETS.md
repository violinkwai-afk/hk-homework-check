...
- 🟡 **第13項：喺`/api/mark`加返「AI判斷code搞唔掂」呢一步，等於填返OCR→code→AI呢個設計嘅最後一層。** Plan已send俾用戶，等緊確認先開始起code。要點：一頁入面所有needs_review嘅題一齊send俾AI（連原張相，唔淨係文字，否則圖表題一定判錯），一次過問，用返而家嘅Qwen/DeepSeek兩層。**重要risk已講咗俾用戶**：而家21條入面19條都要行呢一步，即係短期成本/時間會接近返網頁版而家嘅水平，唔係淨係補幾條咁少；長遠code覆蓋多咗先會慢慢減少。等用戶確認呢個成本增幅可以接受、定係想先淨係做「純文字判斷」（唔建議，圖表題會判錯）。
