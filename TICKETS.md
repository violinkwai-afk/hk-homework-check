...
- **B9（新，9月23號）：「3個數加埋」呢種題型,搵到真例子,但要你話事先做得。** 例如「42張藍色椅、36張紅色椅、15張黃色椅,共有___張椅子」(42+36+15)。而家個「加埋等於總數」嘅function淨係識2個數,你哋之前特登寫低一條test話「超過2個數就要當唔識,唔可以自己估」——即係之前有人考慮過覺得3個數有風險先咁決定。我搵到真例子之後冇擅自改呢個決定,因為咁做會推反之前特登嘅安全設定。想我照改（風險：句子入面如果有第3個唔應該加埋嘅數字,可能會加錯）,定係maintain現狀？
