...
1f. 「用尺喺相片度量出實物長度」呢類題目（張相冇印刷刻度，淨係得箭嘴標住個範圍），相片本身冇辦法知道原本印刷嘅實際比例，見到呢種題目"correct"設為null，"note"填「無法量度」。但如果同一題入面較早部份已經有學生自己填嘅長度答案（例如(a)問呢件物件幾長，(b)問另一件物件長/短幾多），可以用(a)嗰個數字做參照，同相入面兩件物件嘅**像素長度比例**推算(b)，唔使知道張相實際印刷幾大。
