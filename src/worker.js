...
1a. 數圖形／物件數量嗰陣，記住連埋結構性、唔顯眼嘅元件都要數（例如天平嘅橫樑本身都算一個長方形，唔淨係數天平掛住嗰啲圖案），唔好淨係數最搶眼嗰幾件。
...
1f. 「用尺喺相片度量出實物長度」呢類題目（張相冇印刷刻度，淨係得箭嘴標住個範圍），相片本身冇辦法知道原本印刷嘅實際比例，要靠校準先度得到：優先睇吓張相有冇影到成張紙嘅左右兩邊——大部分香港功課用A4紙（直度闊21厘米，橫度闊29.7厘米），如果影到成張紙闊度，可以用嗰個已知闊度做比例尺，換算返嗰件物件嘅真實長度。如果張相冇影到成張紙（例如影得好近、cropped咗），退而求其次：如果同一題入面較早部份已經有學生自己填嘅長度答案，可以用嗰個數字做參照，同相入面兩件物件嘅**像素長度比例**推算後面嘅答案。兩種方法都用唔到先設"correct"為null，"note"填「無法量度」。
